...
async function jdBeauty() {
  ...
  await getIsvToken2()
  await getToken()
  // $.token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOlwvXC8xMjcuMC4wLjE6OTUwMlwvYXBpXC9hdXRoIiwiaWF0IjoxNjE4NTI3NTUwLCJleHAiOjE2MTg1NzA3NTAsIm5iZiI6MTYxODUyNzU1MCwianRpIjoid3JUREtRVmJCaW1wTldLcSIsInN1YiI6NzA0ODM4LCJwcnYiOiIyM2JkNWM4OTQ5ZjYwMGFkYjM5ZTcwMWM0MDA4NzJkYjdhNTk3NmY3In0.uhM6iI6n9ORRLT5o6F5Qw1iGyJyN_VryyK5hwWItKvU'
  if (!$.token) {
...
async function mr() {
  ...
  let client = new WebSocket(`wss://xinruimz-isv.isvjcloud.com/wss/?token=${$.token}`,null,{
    headers:{
      ...
      'user-agent': 'jdapp;iPhone;9.4.0;14.3;67a220b5d1a98738c8ae0edf27590b61ebe36845;network/wifi;ADID/28457FF5-989E-433E-A2B7-D2F836DF5A39;supportApplePay/0;hasUPPay/0;hasOCPay/0;model/iPhone12,1;addressid/1399534633;supportBestPay/0;appBuild/167541;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1'
    }
  })
    ...
    if (e.data !== 'pong' && e.data && safeGet(e.data)) {
      ...
      switch (vo.action) {
        ...
        case "produce_position_info_v2":
          // console.debug(JSON.stringify(vo))
        ...
        case 'get_benefit':
            let benefit = vo.data
            // console.debug(JSON.stringify(benefit))
            if (benefit) {
              for (let i = 0; i < benefit[0].day_limit - parseInt(benefit[0].day_exchange_count); i++) {
                client.send(`{"msg":{"type":"action","args":{"benefit_id":${benefit[0].id}},"action":"to_exchange"}}`)
                console.log(`兑换 ${benefit[0].description}`)
                await $.wait(1000)
              }
              if ($.total > parseInt(benefit[1].coins)) {
                client.send(`{"msg":{"type":"action","args":{"benefit_id":${benefit[1].id}},"action":"to_exchange"}}`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            console.log('functionId=genToken:',$.isvToken)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            $.token2 = data['token']
            console.log('functionId=isvObfuscator:',$.token2)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
